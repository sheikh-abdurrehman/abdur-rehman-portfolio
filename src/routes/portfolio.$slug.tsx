...
export const Route = createFileRoute("/portfolio/$slug")({
  ...
  head: ({ loaderData }) => {
    if (!loaderData) {
      return {
        meta: [{ title: "Project | Abdur Rehman" }, { name: "robots", content: "noindex" }],
      };
    }
    const p = loaderData.project;
    return {
      meta: [
        { title: `${p.name} | Abdur Rehman Portfolio` },
        { name: "description", content: p.overview.slice(0, 155) },
        { property: "og:title", content: `${p.name} | Shopify Portfolio` },
  ...
  return (
    <SiteLayout>
      ...
      <section className="container-x pb-16">
        ...
        <h1 className="mt-6 font-display text-5xl md:text-8xl leading-[0.95] tracking-tight">
          {p.name}
        </h1>
      ...
      <section className="container-x pb-20">
        <div className="grid gap-16 md:grid-cols-[1fr_1.6fr]">
          ...
          <div className="space-y-12">
            ...
            <Block title="Project Goal" body={p.challenge} />
            ...
            <div>
              ...
              <ul className="space-y-3">
                {p.highlights.map((h, i) => (
                  <li
                    key={i}
                    className="text-lg font-display text-foreground/95 border-l-2 border-primary pl-4"
                  >
                    {h}
                  </li>
                ))}
    ...
    <div>
      ...
      <ul className="flex flex-wrap gap-2">
        {items.map((i) => (
          <li
            key={i}
            className="rounded-full border border-border px-3 py-1.5 text-xs text-foreground/85"
          >
            {i}
          </li>
        ))}
